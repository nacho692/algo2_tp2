...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\While{i $<$ ancho} \Comment $\Ogr$(alto x copy($\sigma$)) ancho de veces $\implies$ $\Ogr$(ancho x alto x copy($\sigma$))
				\State j $\leftarrow$ $0$ \Comment $\Ogr$(1)
				\State $res$[i] $\leftarrow$ CrearArreglo(alto) de $\sigma$ \Comment $\Ogr$(1)
				\While{j $<$ alto} \Comment $\Ogr$( copy($\sigma$) ) alto de veces $\implies$  $\Ogr$(alto x copy($\sigma$))
