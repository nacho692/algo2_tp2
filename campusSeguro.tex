...
\begin{Representacion}
  ...
  \Rep[estr]{
	\begin{enumerate}
		...
		\item $\land$ ($\forall c:$ nat)($\forall s:$ conj(placa)) (esta?($\langle c$,$s \rangle$,$e$.sanciones.lista) $\iff$ $s \subseteq$ claves($e$.agentes) \\
		$\yluego$ ($\forall pl:$placa) $pl \in$ $s$ $\impluego$ obtener($pl$,$e$.agentes).sanciones=$c$ $\land$ obtener($pl$,$e$.agentes).pl = $pl$)
	...
	($\forall c$: nat)($\forall s$:conj(nat))(esta?($\langle$c,s$\rangle)$,$e$.sanciones.lista) $\iff$ def?($c$,$e$.sanciones.rapidas)\\
	 $\yluego s$ = obtener($c$,$e$.sanciones.rapidas)
		...
		\item $\land$ ($\forall c$: nat)($\forall s$: conj(placa)) (esta?($\langle$c,s$\rangle)$,$e$.sanciones.lista)\\
		 $\land$ ($\forall pl:$ placa)($pl \in s$ $\land$ def?($pl$,$e$.agentes))) $\impluego$ \\
