...
\begin{Interfaz}
  ...
  $\land$($\forall a_0,a_1$: agente)((def?($a_0$,$d$)$\land$def?($a_1$,$d$)$\land a_0 \neq a_1$)$\impluego$obtener($a_0$,$d$)$\neq$obtener($a_1$,$d$))]
	...
	\InterfazFuncion{ConMismasSanciones}{\In{c}{campusSeg}, \In{a}{agente}}{itConjNoModif(placa)}
	[$a \in Agentes(c)$]
	{esPermutacion(Siguientes($res$),secuenciarConj(conMismasSanciones($a$,$c$)))}
	[$\Ogr(1)$]
	
	\InterfazFuncion{ConKSanciones}{\In{c}{campusSeg}, \In{k}{nat}}{itConjNoModif(placa)}
	[$a \in Agentes(c)$]
...
\begin{algorithmic}[1]
\Procedure{iConKSanciones}{\texttt{in} c : \texttt{estr}, \texttt{in} k : \texttt{nat}} $\to res$:itConjNoModif(placa)
...
\begin{algorithmic}[1]
\Procedure{iConMismasSanciones}{\texttt{in} c : \texttt{estr}, \texttt{in} pl : \texttt{placa}} $\to res$:itConjNoModif(placa)
